...
The overworld runtime SHALL derive dirt-road overlay membership from world-owned deterministic road-network data instead of from render-time heuristics or spawn-centered band logic. The same world seed and world-space coordinates SHALL produce the same road-covered tile coordinates across repeated frames and repeated launches. The published road shape SHALL support topology-backed branches, loops, segment endpoints, and destination-connected approaches while keeping the same occupancy results for world queries and world-content placement rules.
...
- **THEN** the published road occupancy can form branches, loops, and destination-connected approaches in addition to straight segments
- **AND** those occupancy results come from the same world-owned road-network source of truth rather than from hard-coded spawn-cross logic
...
#### Scenario: Topology-backed occupancy composes road motifs without graph-specific art roles
- **WHEN** a deterministic road endpoint, branch junction, loop bend, or destination approach produces an occupancy pattern whose neighboring tiles resolve to coordinated edge and corner roles
