...
The runtime SHALL allow the `World` module to delegate deterministic world-generation policy to dedicated world-owned collaborators so biome sampling, terrain generation, chunk content generation, and similar generation rules can evolve without expanding the gameplay-facing `World` boundary. `World` SHALL remain responsible for owning runtime world state, retained chunk data, and world-facing queries built on that retained state.

#### Scenario: Terrain and content generation policy are separated from runtime world queries
- **WHEN** the overworld implementation adds or updates deterministic terrain or chunk-content generation rules
...
- **AND** `World` remains responsible for exposing world-facing queries such as traversability, tile lookup, metadata access, spawn access, and chunk content access from its retained runtime state
