...
- **AND** it renders text for frame rate, retained chunk count, retained generated-content count, rendered generated-content count, visible tile count, visible generated-content count, front occluder count, terrain vertex count, tile-grid vertex count, player tile coordinates, and zoom level on top of that panel
...
#### Scenario: Overlay reports render-batch scale
- **WHEN** the debug overlay is visible for an overworld frame with terrain and generated content prepared for drawing
- **THEN** the overlay reports the visible tile count and terrain vertex count for the active frame
- **AND** it reports the front occluder count derived from the current render queue
