...
The runtime SHALL preserve vegetation anchor placement metadata from the staged vegetation classification, including `placeOn` tile constraints and `biomes` weights, so vegetation placement can use the staged asset data as its source of truth.
...
- **THEN** a vegetation prototype such as a water lily retains its allowed anchor tile classes and biome weights
...
- **THEN** the runtime retains the prototype's allowed anchor tile classes and biome weights from the staged classification
