...
#### Scenario: Topology publishes structural semantics for richer layouts
- **WHEN** the world generates a deterministic road network for a seed that selects richer layouts
...
- **WHEN** a tile falls within the stamped structure produced for a deterministic road junction, branch meeting, endpoint, or plaza
- **THEN** road occupancy queries report that tile as road-covered
