...
Define the deterministic road-network topology owned by the world runtime and the topology-projected road occupancy shared across systems.

## Requirements

### Requirement: World owns a deterministic road network topology
The world runtime SHALL derive a small road network from deterministic world-owned data rather than from render-time heuristics. The network SHALL contain stable road nodes and connecting segments suitable for representing spawn-adjacent topology such as junctions, endpoints, branches, loops, and optional destination anchors. The same world seed SHALL produce the same road-network topology across repeated launches.
...
- **AND** those features remain encoded as nodes and segment connections rather than as hard-coded occupancy special cases

### Requirement: Road occupancy is projected from topology footprints
The world runtime SHALL answer road occupancy by projecting the deterministic road network into tile-space footprints for segments, junctions, endpoints, and optional destination centers. Tiles covered by those topology-derived footprints SHALL be treated as road-covered consistently across world queries, visible road publication, and road-aware content exclusion.

#### Scenario: Segment footprints publish road-covered tiles
- **WHEN** a tile falls within the footprint of a deterministic road segment
...
- **WHEN** a tile falls within the footprint of a deterministic road junction, branch meeting, or endpoint
- **THEN** road occupancy queries report that tile as road-covered
- **AND** the result does not require special render-only logic outside the shared road occupancy source of truth
...
- **THEN** they use the same topology-backed occupancy decision
